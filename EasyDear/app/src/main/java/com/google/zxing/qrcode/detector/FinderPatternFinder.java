...
public class FinderPatternFinder {
    ...
    private final int[] crossCheckStateCount;//相同的像素放在一块，目的是为了查找 1:1:3:1:1 比例数据，所以创建了长度为5的数组
    ...
    /**
     * 1.在这一行中将连续的相同颜色的像素个数计入数组中，数组长度为5位， 即去找黑\白\黑\白\黑的图像
     * （如开始检测到黑色计入数组[0]，直到检测到白色之前都将数组[0]的值+1；检测到白色了就开始在数组[1]中计数，
     * 以此类推）。
     * 2.填满5位后检测这5位中像素个数是否比例为1:1:3:1:1（可以有50%的误差范围），
     * 3.如果满足条件就说明找到了定位符的大概位置，将这个图像交给handlePossibleCenter方法去找到定位符的中心点，
     * 4.handlePossibleCenter方法是先从垂直方向检测是否满足定位符的条件，如满足就定出Y轴的中心点坐标值，
     * 5.然后用这个坐标值去再次检测水平方向是否满足定位符条件，如满足就定出X轴的中心点坐标值。
     * 至此就找到了一个定位符的中心坐标。
     *
     * @param hints
     * @return
    ...
    final FinderPatternInfo find(Map<DecodeHintType, ?> hints) throws NotFoundException {
        boolean tryHarder = hints != null && hints.containsKey(DecodeHintType.TRY_HARDER);//是否启动精准模式
        boolean pureBarcode = hints != null && hints.containsKey(DecodeHintType.PURE_BARCODE);//是否是纯黑白的
        int maxI = image.getHeight();//高度
        int maxJ = image.getWidth();
        // 开始寻找白/黑/白/黑/白 1:1:3:1:1比例的模块
        ...
        boolean done = false;
        int[] stateCount = new int[5]; // 存储1:1:3:1:1 数组各自像素的长度
        // 在图像中每隔iSkip就采样一行
        for (int i = iSkip - 1; i < maxI && !done; i += iSkip) {
            ...
            for (int j = 0; j < maxJ; j++) {
                if (image.get(j, i)) {
                    ...
                    } else { // Counting white pixels  计算白色像素总量
                        stateCount[currentState]++;
                    }
                }
            }
     ...
     * @return true if the proportions of the counts is close enough to the 1/1/3/1/1 ratios
     * used by finder patterns to be considered a match
     * 返回是否满足 1/1/3/1/1 这样的比例
     */
    protected static boolean foundPatternCross(int[] stateCount) {
        ...
        float maxVariance = moduleSize *3.0f / 4.0f;//最大方差
//    float maxVariance = moduleSize / 2.0f;//最大方差 原来的计算比例
     ...
     * pattern center
     * 寻找定位符的中心点，方法是先从垂直方向检测是否满足定位符的条件，如满足就定出Y轴的中心点坐标值，
     ...
     * @param pureBarcode true if in "pure barcode" mode 是否为纯条形码？？？
